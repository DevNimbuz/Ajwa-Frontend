...
import { 
  ...
  Menu, Bell, Plus, ArrowRight, ExternalLink
} from 'lucide-react';
...
export default function DashboardPage() {
  ...
  const loadData = async () => {
    try {
      ...
      // Mock notifications for status updates (in real app, this would come from a backend endpoint)
      ...
      <main className="dashboard-main">
        ...
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 32, flexWrap: 'wrap', gap: 20 }}>
          ...
          <div style={{ display: 'flex', gap: 12, position: 'relative' }}>
             ...
             <Link href="/contact" className="btn btn-primary btn-sm" style={{ padding: '0 24px', height: 48 }}>
        ...
        <div className="grid grid-3" style={{ marginBottom: 40 }}>
          <StatCard 
            label="Active Trips" 
            value={trips.booked.length} 
            icon={Plane} 
            color="#059669" 
          />
          <StatCard 
            label="My Inquiries" 
            value={trips.all.length} 
            icon={MessageSquare} 
            color="#3b82f6" 
          />
          <StatCard 
            ...
            <section className="glass-card" style={{ padding: '24px' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                {[
                  ...
                  { label: 'Browse Tour Packages', icon: Plane, href: '/package', desc: 'Find your next trip' },
                  { label: 'Submit New Inquiry', icon: MessageSquare, href: '/contact', desc: 'Get a custom quote' },
                ].map((action, i) => (
                  <Link 
                    key={i} 
                    href={action.href}
        ...
        <button 
          ...
          className="dashboard-fab flex-center shadow-gold"
          style={{
            position: 'fixed', bottom: 32, right: 32,
            width: 60, height: 60, borderRadius: '50%',
            background: 'var(--gradient-gold)', color: '#fff',
            zIndex: 100, border: 'none', transition: 'all 0.3s'
          }}
